/** react-step-wizard From https://github.com/jcmcneal/react-step-wizard
 * Edited to fix some errors from the lib while they merge soution PR
 */
import React, { PureComponent } from 'react';
import PropTypes from 'prop-types';

import Animate from './animate.snippet.css';
...
export default class StepWizard extends PureComponent {
  ...
  isReactComponent = component =>
    component && (typeof component.type === 'function' || typeof component.type === 'object');

  /** Render */
  render() {
    /**
     * Fix for https://github.com/jcmcneal/react-step-wizard/issues/55
     */
    ...
    const childrenWithProps = React.Children.map(this.props.children, (child, i) => {
      ...
      // is an actual element && (Not Lazy Mount || isLazyMount && isActive)
      if (child && (!this.props.isLazyMount || (this.props.isLazyMount && props.isActive))) {
        return <Step {...props}>{this.isReactComponent(child) ? React.cloneElement(child, props) : child}</Step>;
      }
