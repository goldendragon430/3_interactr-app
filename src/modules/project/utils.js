...
export function generateEmbedCode(project) {
  ...
  if (
    ...
    !project.published_path.startsWith(
      "https://swiftcdn6.global.ssl.fastly.net"
    )
  ) {
    return `<script type="text/javascript" src="${wrapperUrl}" async></script>
            <div class="iv-player_responsive_padding" style="padding:${paddingTop} 0 0 0;position:relative;" data-hash="${
      project.storage_path?.split("/")[1]
    }"">
  ...
  if (project.storage_path) {
    ...
    return `<iframe class="_vs_ictr_player" src="https://swiftcdn6.global.ssl.fastly.net/${
      project.storage_path
    }/index.html?cb=${randomString()}" width=${project.embed_width} height=${
      project.embed_height
    } frameborder="0" allow="autoplay *" scrolling="no" ></iframe><script src="${wrapperUrl}"></script>`;
  }
...
export function generateEmbedCodeForPreviewing(project, startNodeId) {
  ...
  return `<div 
            ...
            style="padding:${paddingTop} 0 0 0;position:relative;width:${
    project.embed_width
  }px; height:${project.embed_height}px;" 
