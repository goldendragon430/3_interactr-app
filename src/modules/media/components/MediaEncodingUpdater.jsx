...
const Error = ({error = null}) => {
  ...
  return <Label onClick={handleRetry}  danger style={{marginTop: 2}} small>Error! Click to Retry</Label>;
}
...
const ShowLabelAndPollForUpdates = ({item}) => {
    ...
    const getUpdate = async ()=>{
     try {
       ...
       if(json.status===4) {
         ...
         // Call the func again in 30secs until we get a sattus of 4
  ...
  if(status === 0) return <Label  flash primary style={{marginTop: 2}} small>Created...</Label>

  if(status === 1) return <Label flash secondary style={{marginTop: 2}} small>Uploading...</Label>

  if(status === 2) return <Label flash purple style={{marginTop: 2}} small>Queued...</Label>

  if(status === 3) return <Label flash purple style={{marginTop: 2}} small>Transcoding...</Label>

  if(status === 4) return <Label primary style={{marginTop: 2}}>Completed!</Label>
