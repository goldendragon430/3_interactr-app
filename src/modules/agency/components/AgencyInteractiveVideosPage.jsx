...
import {useAgencyProjectsRoute} from "../routeHooks";
...
const AgencyInteractiveVideosPage = ({breadcrumb, heading}) => {
...
export default AgencyInteractiveVideosPage;

const PageWrapper = ({children}) => {

  const [{page, search: q}, setGroupParams] = useAgencyProjectsRoute();
  const [search, setSearch] = useState(q);

  const updateTemplatesList = (options = {page: 1, search: ""}) => {
      setGroupParams(options);
  };
          ...
          <p style={{marginBottom: '20px'}}>
            <div className={filterInputStyles.wrapper} >
              <TextInput  
                ...
                onKeyPress={({key}) => key === 'Enter' ? setGroupParams({search, page}) : null }
              />
              <Icon name="search" onClick={() => setGroupParams({search, page})} />
            </div>
          </p>
        </div>
